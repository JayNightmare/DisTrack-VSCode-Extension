...
export class DiscordCodingViewProvider implements vscode.WebviewViewProvider {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      switch (message.command) {
        ...
        case "reconnectDiscord":
          await vscode.commands.executeCommand("extension.reconnectDiscord");
          await this.updateWebview();
          break;
        case "refreshRPC":
          await vscode.commands.executeCommand("extension.refreshRPC");
          break;
      }
    });
  }
  ...
  private async getHtmlContent(
    ...
    const previousRanks =
      this._context.globalState.get<Record<string, number>>("previousRank") ||
      {};
