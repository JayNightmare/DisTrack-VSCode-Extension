...
export function getGlobalStateValue<T>(key: string): T | undefined {
    return extensionContext?.globalState.get<T>(key);
}

export async function updateGlobalStateValue<T>(
    key: string,
    value: T | undefined
): Promise<void> {
    if (!extensionContext) {
        console.warn(
            `<< Attempted to update global state '${key}' before activation >>`
        );
        return;
    }
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    try {
        const storedApiToken = await context.secrets.get("distrack.apiToken");
        initializeApiToken(storedApiToken ?? null);
    } catch (error) {
        console.warn("<< Failed to rehydrate API token from secrets >>", error);
    }
                        ...
                        if (linkCode && /^[A-Z0-9]{6}$/.test(linkCode)) {
                            ...
                            try {
                                ...
                                if (result.success && result.userId) {
                                    ...
                                    // Ensure we have a persistent device identifier
                                    const deviceId =
                                    ...
                                    try {
                                        ...
                                        vscode.window.showErrorMessage(
                                            "Linked account, but retrieving access token failed. Please try linking again."
                                        );
                                    }
