...
export async function activate(context: vscode.ExtensionContext) {
  ...
  configManager.onConfigurationChanged((e) => {
    if (
      configManager.hasConfigurationChanged(e, "extension.sessionTimerFormat")
    ) {
...
async function beginLinkFlow(): Promise<void> {
  ...
  try {
    ...
    const { linkCode, pollToken, verificationUrl, expiresIn, pollInterval } =
...
export async function deactivate() {
  ...
  if (duration) {
    ...
    try {
      ...
      console.warn("<< Unable to fetch streak data during shutdown >>", error);
    }
